...
function createAudioCard(soundData) {
  ...
  const hasTranscript =
    soundData.transcript && soundData.transcript !== "Not available";
...
function filterCards(searchTerm) {
  ...
  cards.forEach((card) => {
    ...
    const transcript =
      card.querySelector(".transcript p")?.textContent.toLowerCase() || "";
...
function copyShareLink(button, url) {
  ...
  const transcript =
    card.querySelector(".transcript p")?.textContent ||
  ...
  navigator.clipboard
    ...
    .then(() => {
      ...
      // Update tooltip text
      tooltip.textContent =
        "Copied Discord-friendly share links to your clipboard";
