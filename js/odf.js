...
class ODFBrowser {
      ...
      if (compareParam) {
        ...
        if (odf1 && odf2) {
          ...
          for (const [category, odfs] of Object.entries(this.data)) {
            ...
            const odf1Exact = Object.keys(odfs).find(
              (filename) =>
                filename.toLowerCase() === `${odf1.toLowerCase()}.odf`
            );
            const odf2Exact = Object.keys(odfs).find(
              (filename) =>
          ...
          const odfItem = document.querySelector(
            `.odf-item[data-filename="${odf1Match.filename}"]`
          );
          if (odfItem) {
            const categoryTab = document.querySelector(
              `#sidebar-tab-${odf1Match.category}`
            );
            if (categoryTab) {
              categoryTab.click();
            }
  ...
  async loadData() {
    try {
      const response = await fetch("/BZCC-Website/data/odf/odf.min.json");
  ...
  initializeSidebar() {
    ...
    document.querySelectorAll("#categoryTabs .nav-link").forEach((navLink) => {
      navLink.addEventListener("click", () => {
        ...
        document
          .querySelectorAll("#categoryContent .tab-pane")
          .forEach((pane) => {
  ...
  generateODFList(category, odfs) {
    ...
    const [namedOdfs, unnamedOdfs] = entries.reduce(
      (result, [filename, data]) => {
        const displayName =
          data.GameObjectClass?.unitName || data.WeaponClass?.wpnName;
    ...
    const sortedUnnamedOdfs = unnamedOdfs.sort(([a], [b]) =>
      a.localeCompare(b)
    );
      ...
      .map(([filename, data]) => {
        ...
        return `
                    <button class="list-group-item list-group-item-action odf-item" 
                        ...
                        ${
                          displayName
                            ? `<span class="odf-name">${displayName}</span> <span class="ms-2 text-secondary">${filename.replace(
                                ".odf",
                                ""
                              )}</span>`
  ...
  updateBadgeStyles() {
    document.querySelectorAll("#categoryTabs .nav-link").forEach((tab) => {
      ...
      if (badge) {
        ...
        badge.classList.add(
          tab.classList.contains("active") ? "bg-primary" : "bg-dark"
        );
      }
    });
  }
  ...
  handleSearch(term) {
    ...
    if (matchToSelect) {
      ...
      if (firstExactMatch && exactMatchCategory) {
        const targetTab = document.querySelector(
          `#sidebar-tab-${exactMatchCategory}`
        );
    ...
    Object.entries(categoryCounts).forEach(([category, count]) => {
      ...
      tab.innerHTML =
        count > 0
          ? `${category} <div><span class="badge ${
              isActiveCategory
                ? "bg-primary rounded-0 px-2"
                : "bg-dark rounded-0 px-2"
            }">${count}</span></div>`
          : category;
    });
    ...
    if (!firstExactMatch) {
      const bestCategory = Object.entries(categoryCounts).reduce(
          ...
          count > best.count ? { category, count } : best,
        {
          ...
          count: categoryCounts[this.currentCategory] || 0,
        }
      );

      if (
        bestCategory.count > 0 &&
        bestCategory.category !== this.currentCategory
      ) {
  ...
  displayODFData(category, filename) {
    ...
    const displayName =
      odfData.GameObjectClass?.unitName ||
      odfData.WeaponClass?.wpnName ||
      filename;
    const inheritanceHtml = odfData.inheritanceChain
      ? `<div class="text-info small">Inherits: ${odfData.inheritanceChain.join(
          " → "
        )}</div>`
      : "";
    ...
    const entries = Object.keys(odfData).filter(
      (name) => name !== "inheritanceChain"
    );
    ...
    document
      .querySelectorAll('#odfContentContent [data-bs-toggle="pill"]')
      .forEach((navLink) => {
  ...
  formatValue(value, propertyName = "", category = "", odfData = null) {
    if (typeof value === "object" && value !== null) {
      return `<pre class="mb-0"><code>${JSON.stringify(
        value,
        null,
        2
      )}</code></pre>`;
    ...
    if (propertyName === "engageRange" && category === "Vehicle") {
      ...
      const craftClass = Object.entries(odfData).find(
        ([className]) => className === "CraftClass"
      )?.[1];
      ...
      if (craftClass) {
        ...
        if (thirdLetter === "S") {
          popoverContent =
            "Per the <code>subAttackClass</code>, this unit uses its <code>Weapon.aiRange</code> property value instead of <code>engageRange</code>";
        } else if (thirdLetter === "N") {
          popoverContent =
            "Per the <code&lt;subAttackClass</code&lt;, this unit uses this engageRange value instead of its Weapon aiRange";
        }
  ...
  cycleODFs(direction) {
    ...
    const activeTabPane = document.querySelector(
      "#categoryContent .tab-pane.active"
    );
  ...
  selectODFByName(odfName, targetCategory = null) {
    ...
    for (const [category, odfs] of Object.entries(this.data)) {
      if (normalizedName in odfs) {
        ...
        const odfItem = document.querySelector(
          `.odf-item[data-filename="${normalizedName}"]`
        );
        if (odfItem) {
          ...
          if (targetCategory) {
            console.log(
              "Attempting to select target category after ODF display"
            );
  ...
  initializeEventListeners() {
    ...
    document.addEventListener("click", (e) => {
      if (e.target.matches("a[data-category][data-filename]")) {
        ...
        const odfItem = document.querySelector(
          `.odf-item[data-filename="${filename}"]`
        );
        if (odfItem) {
  ...
  handlePropertySearch(term) {
    const activeTabContent =
      document.querySelector("#odfContentContent .tab-pane.active") ||
    ...
    const filteredEntries = entries
      .map((entry) => {
        ...
        const filteredProperties = Object.entries(classData).reduce(
          (acc, [key, value]) => {
    ...
    const activeTabButton = document.querySelector(
      '#odfContentContent [data-bs-toggle="pill"].active'
    );
    const tabName = activeTabButton
    ...
    if (filteredEntries.length === 0) {
      const alertHtml = `
            ...
            `;
      const contentArea =
        activeTabContent.querySelector(".row") || activeTabContent;
  ...
  renderColumnContent(container, entries) {
    ...
    entries.forEach((entry) => {
      const card =
        entry instanceof Element ? entry : this.formatODFDataColumn([entry]);
      const height =
        entry instanceof Element
          ? 60 +
            42 +
            entry.querySelectorAll("tbody tr:not(.d-none)").length * 42
  ...
  cycleContentTabs(forward = true) {
    const tabs = Array.from(
      document.querySelectorAll('#odfContentContent [data-bs-toggle="pill"]')
    );
  ...
  loadFirstVehicleODF() {
    ...
    if (firstVehicleODF) {
      ...
      const odfItem = document.querySelector(
        `.odf-item[data-filename="${firstVehicleODF}"]`
      );
      if (odfItem) {
        odfItem.classList.add("active");
        odfItem.scrollIntoView({ block: "nearest" });
  ...
  compareODFs(odf1, odf2) {
    ...
    const displayName1 =
      data1.GameObjectClass?.unitName ||
      data1.WeaponClass?.wpnName ||
      odf1.filename;
    const displayName2 =
      data2.GameObjectClass?.unitName ||
      data2.WeaponClass?.wpnName ||
      odf2.filename;
      ...
      odf1.filename
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button class="btn btn-sm btn-outline-secondary" onclick="
                                window.browser.updateURL({ odf: '${odf1.filename.replace(
                                  ".odf",
                                  ""
                                )}' });
                                window.browser.displayODFData('${
                                  odf1.category
                                }', '${odf1.filename}')
                            ">
                                Exit Comparison
  ...
  filterComparisonProperties(term) {
    const { odf1, odf2, data1, data2, displayName1, displayName2 } =
      this.comparisonData;
    ...
    const existingAlert = comparisonContent.querySelector(
      ".alert-primary:not(.position-sticky)"
    );
    if (existingAlert) {
      existingAlert.remove();
    }
    ...
    // Show alert if no matches found
    if (
      Object.keys(filteredData1).length === 0 &&
      Object.keys(filteredData2).length === 0
    ) {
      content = `
  ...
  formatComparisonColumn(data, otherData, displayName, columnId) {
    ...
    const classTypes = new Set(
      [...Object.keys(data), ...Object.keys(otherData)].filter(
        (key) => key !== "inheritanceChain"
      )
    );
    ...
    for (const classType of classTypes) {
      ...
      if (!Object.keys(classData).length && !Object.keys(otherClassData).length)
        continue;
      ...
      const createRow = (prop, value, type) => {
        ...
        return `
                    <tr data-property="${prop}" data-class="${classType}" data-column="${columnId}">
                        ...
                        <td class="${classes[type]}">${this.formatValue(
          value,
          prop
        )}</td>
                    </tr>
      ...
      propertyGroups.different.forEach((item) =>
        rows.push(createRow(item.prop, item.value, item.type))
      );
      propertyGroups.same.forEach((item) =>
        rows.push(createRow(item.prop, item.value, item.type))
      );
      propertyGroups.unique.forEach((item) =>
        rows.push(createRow(item.prop, item.value, item.type))
      );
    ...
    return (
      html.join("") || '<div class="alert alert-info">No data to display</div>'
    );
  }
  ...
  handleCompareState(odf1, odf2) {
    ...
    for (const [category, odfs] of Object.entries(this.data)) {
      const odf1Exact = Object.keys(odfs).find(
        (filename) => filename.toLowerCase() === `${odf1.toLowerCase()}.odf`
      );
      const odf2Exact = Object.keys(odfs).find(
        (filename) => filename.toLowerCase() === `${odf2.toLowerCase()}.odf`
      );
  ...
  equalizeCardHeights() {
    ...
    const classTypes = new Set([
      ...Array.from(document.querySelectorAll("#odf1-column .card")).map(
        (card) => card.dataset.classType
      ),
      ...Array.from(document.querySelectorAll("#odf2-column .card")).map(
        (card) => card.dataset.classType
      ),
    ]);
    ...
    classTypes.forEach((classType) => {
      const card1 = document.querySelector(
        `#odf1-column .card[data-class-type="${classType}"]`
      );
      const card2 = document.querySelector(
        `#odf2-column .card[data-class-type="${classType}"]`
      );
  ...
  initializeBuildTree() {
    ...
    modalElement.addEventListener("shown.bs.modal", () => {
      this.generateBuildTree(
        "ibrecy_vsr",
        document.getElementById("isdfBuildTree")
      );
      this.generateBuildTree(
        "ebrecym_vsr",
        document.getElementById("hadeanBuildTree")
      );
      this.generateBuildTree(
        "fbrecy_vsr",
        document.getElementById("scionBuildTree")
      );
    });
  }
  ...
  generateBuildTree(odfName, container) {
    ...
    for (const [cat, odfs] of Object.entries(this.data)) {
      const exactMatch = Object.keys(odfs).find(
        (fname) =>
    ...
    const hasCollapsibleChildren =
      (odfData.ConstructionRigClass ||
        odfData.FactoryClass ||
        odfData.ArmoryClass ||
        isBaseODF) &&
      children.length > 0;
    ...
    node.innerHTML = `
            <div class="d-flex align-items-center gap-2">
                <div class="alert alert-${this.getCategoryColor(
                  category,
                  odfData
                )} d-inline-block ${
      category === "Weapon" ? "p-0 px-2" : "py-2"
    }" 
                     data-category="${category}" 
                     data-filename="${filename}"
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ${
                          properties.unitName
                            ? `<span class="fw-bold">${properties.unitName}</span>`
                            : ""
                        }
                    </div>
                    <div class="collapse" id="${collapseId}"
                        ...
                        <div class="pt-2 border-top border-opacity-25 mt-2 pb-2">
                            ...
                            <button class="btn btn-sm btn-${this.getCategoryColor(
                              category,
                              odfData
                            )}" onclick="
            ...
            ${
              children.length
                ? `<div class="build-tree-children ps-4 mt-2 border-start border-secondary collapse ${
                    !hasCollapsibleChildren || isBaseODF ? "show" : ""
                  }" id="${childrenCollapseId}"></div>`
                : ""
            }
        `;
    ...
    if (category === "Vehicle") {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        if (weaponFile) {
          ...
          weaponCounts.set(
            displayName,
            (weaponCounts.get(displayName) || 0) + 1
          );
        }
      }
  ...
  findChildODFs(odfData, category, filename) {
    ...
    if (gameObj.upgradeName) {
      ...
      if (
        gameObj.baseName?.toLowerCase() === "ebfact" || // Hadean Xenomator
        gameObj.baseName?.toLowerCase() === "fbkiln" || // Scion Kiln
        filename.toLowerCase().includes("fbkiln_vsr")
      ) {
        // Additional check for Scion Kiln
  ...
  formatTreeProperties(properties) {
    ...
    for (const [key, value] of Object.entries(properties)) {
      ...
      if (key === "weapons") {
        ...
        html.push(
          `<div><span class="opacity-75">${key}:</span> ${value}</div>`
        );
      }
    }
