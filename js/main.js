...
const baseSteamProtocol =
  "steam://rungame/624970/76561198955218468/-connect-mp%20";
...
if (hostString) {
  if (hostString.toLowerCase() === "true") {
    window.location.replace(
      'steam://rungame/624970/76561198955218468/-hostname%20"bz2vsr"%20-nomovies'
    );
  } else {
    alert(
      "Invalid input. Use bz2vsr.com/?host=true if you are trying to host a game."
    );
  }
  window.location.href = "/";
}
...
function stringToHex(str) {
  return Array.from(str)
    .map((char) => charToHex(char.charCodeAt(0)))
    .join("");
}
  ...
  fetch(site_root + "data/maps/vsrmaplist.json")
    ...
    .then((MapData) => {
      ...
      Maps.forEach((map, index) => {
        document.querySelector(`#pickerMapTitle-${index}`).textContent =
          map.Name;
        document.querySelector(`#pickerMapImage-${index}`).src = map.Image;
        document.querySelector(`#pickerMapPools-${index}`).textContent =
          map.Pools;
        document.querySelector(`#pickerMapSize-${index}`).textContent =
          map.Size.baseToBase;
        document.querySelector(`#pickerMapLoose-${index}`).textContent =
          map.Loose === -2 ? "INF" : map.Loose === -1 ? "NA" : map.Loose;
      });
    })
...
function renderPlayerCard(
  ...
  const isCommanderSlot =
    (player.Team && player.Team.Leader === true) ||
    player.slot === 1 ||
    player.slot === 6;
    ...
    return `
        <li class="list-group-item d-flex justify-content-between align-items-center ${warningClass} ${bgClass} no-hover player-card">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <span style="${isCommanderSlot ? "opacity: 0.85;" : ""}">${
      isCommanderSlot ? "⌘ No Commander" : player.Name
    }</span>
                </div>
            </div>
        </li>`;
  ...
  let steamAvatarUrl =
    player.Name === "Computer"
      ? site_root + "img/computer.png"
      : site_root + "img/no_steam_pfp.jpg";
  ...
  if (
    player.IDs &&
    player.IDs.Steam &&
    player.IDs.Steam.ID !== undefined &&
    player.IDs.Steam.ID !== null
  ) {
    ...
    if (SteamPlayerList && SteamPlayerList[steamId]) {
      ...
      steamAvatarUrl =
        SteamPlayerList[steamId].AvatarUrl ||
        site_root + "img/no_steam_pfp.jpg";
    }
  }
  // Handle GOG players
    ...
    player.IDs &&
    player.IDs.Gog &&
    player.IDs.Gog.ID !== undefined &&
    player.IDs.Gog.ID !== null
  ) {
    ...
    if (GogPlayerList && GogPlayerList[gogId]) {
      ...
      steamAvatarUrl =
        GogPlayerList[gogId].AvatarUrl || site_root + "img/no_steam_pfp.jpg";
    }
  }
  ...
  const showInGameName =
    (steamNickname && steamNickname !== playerName) ||
  ...
  let cardContent = `
        <li class="list-group-item d-flex justify-content-between align-items-center${
          player.Name === "Computer" ? " no-hover computer-team" : ""
        } player-card" data-player='${playerData}'>
            <div class="d-flex align-items-center">
                ${
                  !compactPlayerCards
                    ? `<img src="${steamAvatarUrl}" class="me-2 img-thumbnail" width="48" height="48" onError="this.src='${site_root}img/no_steam_pfp.jpg'">`
                    : ""
                }
                <div>
                    <span class="text-light fw-bold">
                        ${
                          isCommanderSlot
                            ? `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-command me-1 mb-1" viewBox="0 0 16 16">
            ...
            ${
              !compactPlayerCards && playerScore !== ""
                ? `<span class="badge bg-secondary">${playerScore}</span>`
                : ""
            }
        </li>`;
...
async function getLobbyData() {
  ...
  let sourceURL =
    "https://multiplayersessionlist.iondriver.com/api/1.0/sessions?game=bigboat:battlezone_combat_commander";
  ...
  try {
    ...
    let fetchMapDataResponse = await fetch(
      site_root + "data/maps/vsrmaplist.json"
    );

    if (!fetchResponse.ok || !fetchMapDataResponse.ok) {
      console.log(
        `Error with response. Make sure source and proxy URLs are accessible and returning valid data.`
      );
    }
    ...
    if (
      JSON.stringify(data.DataCache) === "{}" ||
      (data.DataCache.Players.IDs.Steam === undefined &&
        data.DataCache.Players.IDs.GOG === undefined)
    ) {
    ...
    if (SteamPlayerList)
      OnlinePlayers += Object.entries(SteamPlayerList).length;
    ...
    for (let i = 0; i < GameList.length; i++) {
      ...
      Players.forEach(function (player) {
        ...
        if (
          ...
          for (const [SteamID, SteamData] of Object.entries(SteamPlayerList)) {
            let SteamNick = SteamData.Nickname
              ? SteamData.Nickname.toString()
              : "";
      ...
      if (currentActivePlayerCount > maxActivePlayerCount) {
        console.log(
          '%cMarking "' + game.Name + '" as BZ2 Vet Strat',
        ...
        console.log(
          "%cGame has active players (" +
            currentActivePlayerCount +
            "), but not more than current max (" +
    ...
    GameList.forEach((game, index) => {
      ...
      let gameMode =
        game.Level.GameMode === undefined ? "N/A" : game.Level.GameMode.ID;
      let gameVersion = game.Game.Version;
      let gameMod = game.Game.Mod;
      let gameModName =
        gameMod !== undefined
          ? Mods[gameMod] !== undefined
            ? Mods[gameMod].Name
            : "Unknown Mod"
          : "Stock";
      let gameTime = game.Time.Seconds / 60;
      let gameMessage =
        game.Message !== undefined ? clean(game.Message) : "No game message";
      let gameState =
        game.Status.State === "PreGame"
          ? "In-Lobby"
      ...
      if (
        localStorage.getItem("ShowVSROnly") === "true" ||
        document.querySelector("#VSRToggle").checked
      ) {
        if (gameMod !== vsrModID) {
          return;
        }
      ...
      if (gameMod !== undefined && !hasPassword && !isLocked) {
                ...
                */
        {
          const textareaElement = document.querySelector(
            `button[data-join-string="${encodedArgs}"] textarea`
          );
          if (textareaElement) {
      ...
      if (hasActivePlayers && index === 0) {
        document.title = `${gameState}: ${playerCount}/${playerCountMax} (Host: ${clean(
          gameHost
        )})`;
      ...
      if (gameMode === "STRAT") {
        ...
        let sortedPlayers = PlayerList.sort((a, b) => {
          ...
          return parseInt(a.Team.SubTeam.ID) < parseInt(b.Team.SubTeam.ID)
            ? -1
            : 1;
        });
        ...
        `
                    ...
                    <div class="card h-100 border-secondary" style="--bs-border-opacity: .15;">
                        ...
                        <div class="card-header d-flex justify-content-between align-items-center bg-dark-subtle shadow-lg">
                            ...
                            <span>
                                ...
                                <span id="gameState" class="btn btn-sm ${
                                  gameState === "In-Lobby"
                                    ? "bg-secondary"
                                    : "bg-success"
                                } bg-gradient btn-dead">${gameState}</span>
                                ${(() => {
                                  if (hasJoinURL) {
                                    return `
                                        <span class="">
                                        ...
                                        ${
                                          mapVSRObject && isVetStrat
                                            ? `vsr-cursor-pointer img-map" data-bs-toggle="modal" data-bs-target="#mapModal" 
                                                ...
                                                data-bs-map-b2b="${
                                                  mapVSRObject.Size.baseToBase
                                                }" 
                                                data-bs-map-binary="${
                                                  mapVSRObject.Size.binarySave
                                                }"
                                                data-bs-map-loose="${
                                                  mapVSRObject.Loose
                                                }" 
                                                ...
                                                data-bs-map-description="${escapeHtml(
                                                  mapVSRObject.Description
                                                )}" 
                                                data-bs-map-author="${
                                                  mapVSRObject.Author
                                                }"
          ...
          mapName ? mapName : "N/A"
                                        ...
                                        <li class="list-group-item d-flex justify-content-between align-items-center border-dotted">
                                            ...
                                            <span class="text-light fw-bold">${truncate(
                                              gameHost,
                                              32
                                            )}
                                            ${(() => {
                                              for (const [
                                                ...
                                                if (
                                                  PlayerList[0].IDs !==
                                                    undefined &&
                                                  PlayerList[0].IDs.Steam !==
                                                    undefined
                                                ) {
                                                  if (
                                                    PlayerList[0].IDs.Steam.ID.toString() ===
                                                    SteamID.toString()
                                                  ) {
                                                    // only show the host's steam name if it's different from in-game nick
                                                    if (
                                                      gameHost != Steam.Nickname
                                                    ) {
                                            ...
                                            return `
                                                <div class="col-12 p-2 py-0">
                                                    <div class="alert alert-danger mt-2 mb-0 d-flex align-items-center">
                                                        ...
                                                        <div class="ms-2">
                                                            <strong>Hidden:</strong> ${Hidden.map(
                                                              (player) => {
                                                                let displayName =
                                                                  clean(
                                                                    player.Name
                                            ...
                                            <div class="card h-100 border-secondary-subtle">
                                                ...
                                                <div class="card-body p-0">
                                                    ...
                                                    ${Team1.map((player) => {
                                                      ...
                                                      playerWithTeamID.Name =
                                                        displayName;
                                                      playerWithTeamID.OriginalName =
                                                        inGameName;
    ...
    if (joinBtns !== null) {
      joinBtns.forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          window.location.replace(
            "discord:///channels/1137584338310017098/1137619558639874129"
          );
    ...
    if (mapModal) {
      mapModal.addEventListener("show.bs.modal", (event) => {
        ...
        mmB2B.innerHTML =
          mapBinarySave === "true"
            ? '<span class="text-danger">N/A (Binary Save)</span>'
            : mapB2B;
...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  if (
    localStorage.getItem("LiveUpdatesOn") == "true" ||
    document.querySelector("#LiveUpdateToggle").checked
  ) {
